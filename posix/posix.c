...
int posix_mkfifo(const char *pathname, mode_t mode)
{
	...
	ret = posix_create(pathname, 2 /* otDev */, mode | (unsigned int)S_IFIFO, oid, &file);
	if (ret < 0) {
		return ret;
	}
	...
	if (err == 0) {
		switch ((int)f->type) {
			case ftInetSocket:
				...
				break;
			case ftUnixSocket:
				err = unix_setfl(f->oid.id, (unsigned)val);
				break;
			default:
...
int posix_socketpair(int domain, int type, int protocol, int sv[2])
{
	...
	if (err == 0) {
		...
		if (((unsigned)type & SOCK_CLOEXEC) != 0U) {
...
int posix_accept4(int socket, struct sockaddr *address, socklen_t *address_len, int flags)
{
	...
	if (err == 0) {
		switch ((int)f->type) {
			case ftInetSocket:
				err = inet_accept4(f->oid.port, address, address_len, (unsigned)flags);
				if (err >= 0) {
				...
				break;
			case ftUnixSocket:
				err = unix_accept4(f->oid.id, address, address_len, (unsigned)flags);
				if (err >= 0) {
					p->fds[fd].file->type = ftUnixSocket;
					p->fds[fd].file->oid.port = US_PORT;
					p->fds[fd].file->oid.id = (unsigned int)err;
	...
	if (((unsigned)flags & SOCK_CLOEXEC) != 0U) {
		p->fds[fd].flags = FD_CLOEXEC;
	}
...
ssize_t posix_recvfrom(int socket, void *message, size_t length, int flags, struct sockaddr *src_addr, socklen_t *src_len)
{
	...
	if (err == 0) {
		switch ((int)f->type) {
			case ftInetSocket:
				err = inet_recvfrom(f->oid.port, message, length, (unsigned)flags, src_addr, src_len);
				break;
			case ftUnixSocket:
				err = unix_recvfrom(f->oid.id, message, length, (unsigned)flags, src_addr, src_len);
				break;
			default:
				err = -ENOTSOCK;
				break;
...
ssize_t posix_sendto(int socket, const void *message, size_t length, int flags, const struct sockaddr *dest_addr, socklen_t dest_len)
{
	...
	if (err == 0) {
		switch ((int)f->type) {
			case ftInetSocket:
				err = inet_sendto(f->oid.port, message, length, (unsigned)flags, dest_addr, dest_len);
				break;
			case ftUnixSocket:
				err = unix_sendto(f->oid.id, message, length, (unsigned)flags, dest_addr, dest_len);
				break;
			default:
				err = -ENOTSOCK;
				break;
...
ssize_t posix_recvmsg(int socket, struct msghdr *msg, int flags)
{
	...
	if (err == 0) {
		switch ((int)f->type) {
			case ftInetSocket:
				err = inet_recvmsg(f->oid.port, msg, (unsigned)flags);
				break;
			case ftUnixSocket:
				err = unix_recvmsg(f->oid.id, msg, (unsigned)flags);
				break;
			default:
				err = -ENOTSOCK;
				break;
...
ssize_t posix_sendmsg(int socket, const struct msghdr *msg, int flags)
{
	...
	if (err == 0) {
		switch ((int)f->type) {
			case ftInetSocket:
				err = inet_sendmsg(f->oid.port, msg, (unsigned)flags);
				break;
			case ftUnixSocket:
				err = unix_sendmsg(f->oid.id, msg, (unsigned)flags);
				break;
			default:
				err = -ENOTSOCK;
				break;
	...
	for (i = 0; i < (int)nfds; ++i) {
		...
		if (posix_getOpenFile(fds[i].fd, &f) < 0) {
			...
			if (f->type == ftUnixSocket) {
				err = unix_poll((unsigned)msg.oid.id, fds[i].events);
			}
			else {
