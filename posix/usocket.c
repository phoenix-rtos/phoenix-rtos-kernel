...
static unsigned int _usocket_opFlags(const usocket_t *s, unsigned int flags)
{
	unsigned int op = 0;
...
int usocket_bind(usocket_t *s, const struct sockaddr *address, socklen_t address_len)
{
	...
	if (err == 0) {
		 ...
		 * registered and reachable through the filesystem. A listen() that
...
int usocket_connect(usocket_t *s, const struct sockaddr *address, socklen_t address_len)
{
	...
	while (s->state == (u8)usocketConnecting) {
		...
		/*
		 * POSIX says that connect() shall block for up to an unspecified
...
int usocket_accept4(usocket_t *ls, struct sockaddr *address, socklen_t *address_len, unsigned int flags, usocket_t **s)
{
	...
	for (;;) {
		...
		if ((cs->state != (u8)usocketConnecting) || (cs->rx == NULL)) {
			/*
...
int usocket_poll(usocket_t *s, unsigned short events)
{
	...
	unsigned int ev;
	int state, pending, readShut, writeShut;
	u8 flags;
	...
	/* FIXME: report POLLOUT in usocketUnconnected state, as Linux does? */
	if (state == usocketListening) {
		...
		/*
		 * A direction counts as shut whichever end stopped it: our own
		 * shutdown() records it in the endpoint flags, the peer's in the
		 * channel flags. A datagram socket only ever has the endpoint flag
		 * for the write side, as its transmit channel is shared with other
		 * senders and must not be marked.
		 */
		readShut = ((flags & USOCKET_SHUT_RD) != 0U) ? 1 : 0;
		writeShut = ((flags & USOCKET_SHUT_WR) != 0U) ? 1 : 0;
		...
		if (writeShut != 0) {
			/*
			 * A write that cannot block is writable, whichever end stopped the
			 * direction: it fails with EPIPE at once instead of waiting for
			 * room. The channel of a datagram socket is never marked, so this
			 * is the only place its own shutdown() is seen.
			 */
			revents |= (unsigned int)events & (POLLOUT | POLLWRNORM | POLLWRBAND);
		}

		/*
		 * POLLHUP is a hangup of the whole connection, so it takes both
		 * directions. A half-closed socket keeps working in the direction that
		 * is left, and POLLHUP cannot be masked out - reporting it for a single
		 * shut direction would have event loops tear down usable connections.
		 */
		if ((readShut != 0) && (writeShut != 0)) {
			revents |= POLLHUP;
		}
	}
