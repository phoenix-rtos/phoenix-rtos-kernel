...
void proc_threadsDump(u8 priority)
{
	...
	do {
		t = threads_common.windows[window->id]->ready[priority];
		do {
			...
			t = t->next;
		} while (t != threads_common.windows[window->id]->ready[priority]);
...
int _threads_init(vm_map_t *kmap, vm_object_t *kernel)
{
	...
	threads_common.windows = vm_kmalloc(sizeof(syspage_sched_window_t *) * cnt, NULL);
	...
	threads_common.windowStart = vm_kmalloc(sizeof(*threads_common.windowStart) * hal_cpuGetCount(), NULL);
