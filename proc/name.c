...
int proc_portRegister(u32 port, const char *name, oid_t *oid)
{
	...
	if (_dcache_entryLookup(hash, name) != NULL) {
		(void)proc_lockClear(&name_common.lock);
		if (entry != NULL) {
			vm_kfree(entry, proc_currentPart());
		}
		return -EEXIST;
	}
...
int proc_portUnregister(const char *name)
{
	...
	vm_kfree(entry, proc_currentPart()); /* FIXME: proper partition check after FS separation is implemented */
...
int proc_portLookup(const char *name, oid_t *file, oid_t *dev)
{
	...
	if (entry != NULL) {
		...
		(void)proc_lockClear(&name_common.lock);
		if (pheap != NULL) {
			vm_kfree(pheap, proc_currentPart());
		}
		return EOK;
	}
