...
#define DBG()         hal_consolePrint(ATTR_BOLD, __FILE__ ":" STR(__LINE__) "\n")
...
#endif
	unsigned prot, flags, reloffs;
	int i, j, reltype, badreloc = 0, err;
	void *relptr;
	char *snameTab;
	ptr_t *got;
	...
	for (i = 0, shdr = (void *)((char *)ehdr + ehdr->e_shoff); i < ehdr->e_shnum; i++, shdr++) {
		...
		for (j = 0; j < shdr->sh_size / shdr->sh_entsize; ++j) {
			...
			if (process->fdpic == 0) {
				...
				if (reltype == R_ARM_RELATIVE) {
				...
				else if (reltype == R_ARM_FUNCDESC_VALUE) {
					relptr = (void *)rel->r_offset;
					...
					sym = (Elf32_Sym *)(symTab + (ELF32_R_SYM(rel->r_info) * sizeof(Elf32_Sym)));
...
int process_loadInterpreter(vm_map_t *map, vm_map_t *imap, const void *iehdr, void **interpEntry, struct auxInfo **auxData, int *hasInterpreter, struct elf32_fdpic_loadmap *interpLoadmap)
{
	const Elf32_Ehdr *ehdr = iehdr;
	const char *interpreterPath;
	int err;
	...
	err = process_doLoad(map, imap, interpO, interpIehdr, interpSize, &stacksz, NULL, NULL, NULL, &fdpic, &got, interpEntry, interpLoadmap);
...
int process_load(process_t *process, vm_object_t *o, off_t base, size_t size, void **ustack, void **entry, struct auxInfo **auxData, struct elf32_fdpic_loadmap *loadmap, struct elf32_fdpic_loadmap *interpLoadmap)
{
	...
	err = process_doLoad(process->mapp, process->imapp, o, (void *)base, size, &stacksz, &tlsNew, &tbssAddr, auxData, &process->fdpic, &process->got, entry, loadmap);
	if (err < 0) {
		return err;
	}
	err = process_loadInterpreter(process->mapp, process->imapp, (void *)base, entry, auxData, &process->hasInterpreter, interpLoadmap);
	if (err < 0) {
		return err;
	}
...
static void process_exec(thread_t *current, process_spawn_t *spawn)
{
	...
	struct elf32_fdpic_loadmap_max loadmap, loadmapInterp;
	
	hal_memset(&loadmap, 0, sizeof(loadmap));
	hal_memset(&loadmapInterp, 0, sizeof(loadmapInterp));
	...
	if (err == 0) {
		err = process_load(current->process, spawn->object, spawn->offset, spawn->size, &stack, &entry, &auxDataEnd, (struct elf32_fdpic_loadmap *)&loadmap,  (struct elf32_fdpic_loadmap *)&loadmapInterp);
	}

	if (err == 0) {
		/* TODO: put loadmaps on stack */
		/* TODO: r7 -> loadmap of executable */
		/* TODO: r8 -> loadmap of interpreter */
		/* TODO: r9 -> PT_DYNAMIC of interp */
