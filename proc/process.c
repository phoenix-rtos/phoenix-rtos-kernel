...
static void *proc_copyargs(char **args, partition_t *part)
{
	size_t argc, len = 0U;
	void *storage;
	char **kargs, *p;
...
static int process_copy(void)
{
	...
	if (parent->process->path != NULL) {
		process->path = lib_strdup(parent->process->path, parent->process->partition);
		if (process->path == NULL) {
			return -ENOMEM;
		}
	}
	...
	if (parent->process->argv != NULL) {
		process->argv = proc_copyargs(parent->process->argv, parent->process->partition);
	...
	if (parent->process->envp != NULL) {
		process->envp = proc_copyargs(parent->process->envp, parent->process->partition);
...
int _process_init(vm_map_t *kmap, vm_object_t *kernel)
{
	...
	process_common.partitions = vm_kmalloc(sizeof(partition_t) * cnt, NULL);
	...
	do {
		...
		process_common.partitions[sysPart->id].usedKMem = 0U;
