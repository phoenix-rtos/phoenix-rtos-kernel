...
static void *msg_map(int dir, kmsg_t *kmsg, void *data, size_t size, process_t *from, process_t *to)
{
	...
	if (boffs > 0U) {
		...
		vaddr = vm_mmap(msg_common.kmap, NULL, PHADDR_INVALID, SIZE_PAGE, PROT_READ | PROT_WRITE, VM_OBJ_PHYSMEM, bpa, flags);
		ml->bvaddr = vaddr;
		if (vaddr == NULL) {
			return NULL;
		}
	...
	if (eoffs) {
		...
		vaddr = vm_mmap(msg_common.kmap, NULL, PHADDR_INVALID, SIZE_PAGE, PROT_READ | PROT_WRITE, VM_OBJ_PHYSMEM, epa, flags);
		ml->evaddr = vaddr;
		if (vaddr == NULL) {
			return NULL;
		}
