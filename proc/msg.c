...
static void *msg_map(int dir, kmsg_t *kmsg, void *data, size_t size, process_t *from, process_t *to)
{
	...
	if (boffs != 0U) {
		...
		nbp = vm_pageAlloc(SIZE_PAGE, PAGE_OWNER_APP, from->partition);
		ml->bp = nbp;
		if (nbp == NULL) {
			return NULL;
		}
	...
	if (eoffs != 0U) {
		...
		if ((boffs == 0U) || (eoffs >= boffs)) {
			nep = vm_pageAlloc(SIZE_PAGE, PAGE_OWNER_APP, from->partition);
			ml->ep = nep;
			if (nep == NULL) {
				return NULL;
			}
...
static void msg_release(kmsg_t *kmsg)
{
	...
	if (kmsg->i.bp != NULL) {
		vm_pageFree(kmsg->i.bp, kmsg->src->partition);
	...
	if (kmsg->i.eoffs != 0U) {
		if (kmsg->i.ep != NULL) {
			vm_pageFree(kmsg->i.ep, kmsg->src->partition);
		}
	...
	if (kmsg->o.bp != NULL) {
		vm_pageFree(kmsg->o.bp, kmsg->src->partition);
	...
	if (kmsg->o.eoffs != 0U) {
		if (kmsg->o.ep != NULL) {
			vm_pageFree(kmsg->o.ep, kmsg->src->partition);
		}
