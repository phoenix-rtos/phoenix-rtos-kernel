...
static void *msg_map(int dir, kmsg_t *kmsg, void *data, size_t size, process_t *from, process_t *to)
{
	...
	if (boffs != 0U) {
		...
		nbp = vm_phAlloc(&s, PAGE_OWNER_APP, MAP_CONTIGUOUS, (from == NULL) ? NULL : from->partition);
		ml->bp = nbp;
	...
	if (eoffs != 0U) {
		...
		if ((boffs == 0U) || (eoffs >= boffs)) {
			nep = vm_phAlloc(&s, PAGE_OWNER_APP, MAP_CONTIGUOUS, (from == NULL) ? NULL : from->partition);
			ml->ep = nep;
...
static void msg_release(kmsg_t *kmsg)
{
	...
	if (kmsg->i.bp != PHADDR_INVALID) {
		(void)vm_phFree(kmsg->i.bp, SIZE_PAGE, (kmsg->src == NULL) ? NULL : kmsg->src->partition);
	...
	if (kmsg->i.eoffs != 0U) {
		if (kmsg->i.ep != PHADDR_INVALID) {
			(void)vm_phFree(kmsg->i.ep, SIZE_PAGE, (kmsg->src == NULL) ? NULL : kmsg->src->partition);
		}
	...
	if (kmsg->o.bp != PHADDR_INVALID) {
		(void)vm_phFree(kmsg->o.bp, SIZE_PAGE, (kmsg->src == NULL) ? NULL : kmsg->src->partition);
	...
	if (kmsg->o.eoffs != 0U) {
		if (kmsg->o.ep != PHADDR_INVALID) {
			(void)vm_phFree(kmsg->o.ep, SIZE_PAGE, (kmsg->src == NULL) ? NULL : kmsg->src->partition);
		}
