...
static void coredump_encodeChunk(coredump_state_t *state, const u8 *buf, size_t len)
{
	...
	while (i < len) {
		byte = buf[i];
		state->crc32 = (state->crc32 ^ (byte & 0xFF));
		for (b = 0; b < 8; b++) {
...
void coredump_dumpThreadNotes(coredump_threadinfo_t *threads, size_t threadCnt, coredump_state_t *state, void *buff)
{
	const u32 zero = 0;
	Elf_Nhdr nhdr;
	size_t i;
