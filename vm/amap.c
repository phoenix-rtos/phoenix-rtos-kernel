...
static anon_t *amap_putanon(anon_t *a, syspage_part_t *part)
{
	if (a == NULL) {
		return NULL;
	}
...
static anon_t *anon_new(page_t *p, syspage_part_t *part)
{
	anon_t *a;
...
page_t *amap_page(vm_map_t *map, amap_t *amap, vm_object_t *o, void *vaddr, size_t aoffs, u64 offs, vm_prot_t prot)
{
	...
	amap->anons[aoffs / SIZE_PAGE] = anon_new(p, amap->partition);
