...
static int _map_force(vm_map_t *map, map_entry_t *e, void *paddr, vm_prot_t prot)
{
	...
	if ((p == PHADDR_INVALID) && (e->object == VM_OBJ_PHYSMEM)) {
		// addr_t tmp = pmap_resolve(&map->pmap, paddr);
		// LIB_ASSERT_ALWAYS(tmp == 0U || (tmp & PGHD_PRESENT) == 0U || (tmp & ~(SIZE_PAGE - 1)) == eoffs, "vm_mapForce: Invalid mapping detected! %x != %x", tmp, eoffs);
		if (vm_mappages(&map->pmap, paddr, (addr_t)eoffs, SIZE_PAGE, attr) < 0) {
			return -ENOMEM;
		}
		addr_t tmp = pmap_resolve(&map->pmap, paddr);
		LIB_ASSERT_ALWAYS((tmp & ~(SIZE_PAGE - 1)) == (eoffs & ~(SIZE_PAGE - 1)), "vm_mapForce: mappages error detected! %x != %x", tmp, eoffs);
	}
		...
		// addr_t tmp = pmap_resolve(&map->pmap, paddr);
		// LIB_ASSERT_ALWAYS(tmp == 0U || (tmp & PGHD_PRESENT) == 0U || (tmp & ~(SIZE_PAGE - 1)) == p, "vm_mapForce: Invalid mapping detected! %x != %x", tmp, p);
		if (vm_mappages(&map->pmap, paddr, p, SIZE_PAGE, attr) < 0) {
			amap_putanons(e->amap, e->aoffs + offs, SIZE_PAGE);
			return -ENOMEM;
		}
		addr_t tmp = pmap_resolve(&map->pmap, paddr);
		LIB_ASSERT_ALWAYS((tmp & ~(SIZE_PAGE - 1)) == (p & ~(SIZE_PAGE - 1)), "vm_mapForce: mappages error detected! %x != %x", tmp, p);
	}
	// else {
	// 	/* No action required */
	// }
...
int vm_mapCreate(vm_map_t *map, void *start, void *stop)
{
...
#ifndef NOMMU
	size_t s = 2 * SIZE_PDIR - SIZE_PAGE;  // TODO: a helper in phmap??
	...
	// hal_consolePrint(ATTR_BOLD, b);
	if ((map->pmap.pmapp & (SIZE_PDIR - 1)) != 0) {
		vm_phFree(map->pmap.pmapp, SIZE_PDIR - (map->pmap.pmapp & (SIZE_PDIR - 1)));
		map->pmap.pmapp += SIZE_PDIR - (map->pmap.pmapp & (SIZE_PDIR - 1));
		s -= SIZE_PDIR - (map->pmap.pmapp & (SIZE_PDIR - 1));
	}
	// lib_sprintf(b, "vm_mapCreate: beg %x\n", map->pmap.pmapp);
	// hal_consolePrint(ATTR_BOLD, b);
	if (s > SIZE_PDIR) {
		vm_phFree(map->pmap.pmapp + SIZE_PDIR, s - SIZE_PDIR);
	}
	s = SIZE_PDIR;
	...
	if (map->pmap.pmapv == NULL) {
		vm_phFree(map->pmap.pmapp, s);
		return -ENOMEM;
	}

	pmap_create(&map->pmap, &map_common.kmap->pmap, map->pmap.pmapp, NULL, map->pmap.pmapv);
#else
...
void vm_mapDestroy(process_t *p, vm_map_t *map)
{
	...
	for (;;) {
		...
		vm_phFree(a, SIZE_PAGE);
	}

	vm_munmap(map_common.kmap, map->pmap.pmapv, SIZE_PDIR);
	vm_phFree(map->pmap.pmapp, SIZE_PDIR);
...
static int _map_mapsInit(vm_map_t *kmap, vm_object_t *kernel, void **bss, void **top)
{
	...
	do {
		...
		/* Allocate new map */
			...
			while (((ptr_t)*top) - ((ptr_t)*bss) < (ptr_t)sizeof(vm_map_t)) {
				result = _page_sbrk(&map_common.kmap->pmap, bss, top);
				LIB_ASSERT_ALWAYS(result >= 0, "vm: Problem with extending kernel heap for vm_map_t pool (vaddr=%p)", *bss);
			}
...
int _map_init(vm_map_t *kmap, vm_object_t *kernel, void **bss, void **top)
{
	...
	while ((ptr_t)(*top) - (ptr_t)(*bss) < (ptr_t)sizeof(map_entry_t) * (ptr_t)map_common.ntotal) {
		result = _page_sbrk(&map_common.kmap->pmap, bss, top);
