...
static int _map_force(vm_map_t *map, map_entry_t *e, void *paddr, vm_prot_t prot, syspage_part_t *part);
...
static void map_pageFault(unsigned int n, exc_context_t *ctx)
{
	...
	paddr = (void *)((ptr_t)vaddr & ~(SIZE_PAGE - 1U));
...
int vm_mapCreate(vm_map_t *map, void *start, void *stop)
{
	...
	map->pmap.pmapp = vm_phAlloc(&s, PAGE_OWNER_KERNEL | PAGE_KERNEL_PTABLE, MAP_CONTIGUOUS, NULL);
	...
	if ((map->pmap.pmapp & (SIZE_PDIR - 1U)) != 0U) {
		(void)vm_phFree(map->pmap.pmapp, SIZE_PDIR - (map->pmap.pmapp & (SIZE_PDIR - 1U)), NULL);
	...
	// hal_consolePrint(ATTR_BOLD, b);
	if (s > SIZE_PDIR) {
		(void)vm_phFree(map->pmap.pmapp + SIZE_PDIR, s - SIZE_PDIR, NULL);
	}
	s = SIZE_PDIR;
	...
	if (map->pmap.pmapv == NULL) {
		(void)vm_phFree(map->pmap.pmapp, s, NULL);
		return -ENOMEM;
	}
...
void vm_mapDestroy(process_t *p, vm_map_t *map)
{
	...
	for (;;) {
		...
		(void)vm_phFree(a, SIZE_PAGE, NULL);
	}

	(void)vm_munmap(map_common.kmap, map->pmap.pmapv, SIZE_PDIR);
	(void)vm_phFree(map->pmap.pmapp, SIZE_PDIR, NULL);
	...
	for (n = lib_rbMinimum(src->tree.root); n != NULL; n = lib_rbNext(n)) {
		...
		if ((proc == NULL) || (proc->lazy == 0U)) {
			for (offs = 0; offs < f->size; offs += SIZE_PAGE) {
				...
				if (_map_force(src, e, (void *)((ptr_t)e->vaddr + offs), e->prot, proc != NULL ? proc->partition : NULL) != 0) {
					(void)proc_lockClear(&dst->lock);
					(void)proc_lockClear(&src->lock);
					return -ENOMEM;
				}
