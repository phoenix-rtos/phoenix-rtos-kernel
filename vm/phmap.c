...
addr_t vm_phAlloc(size_t *size, page_flags_t page_flags, vm_flags_t vm_flags, syspage_part_t* partition)
{
	...
	*size = (*size + PH_ALIGN - 1U) & ~(PH_ALIGN - 1U);
	...
	if (partition != NULL) {
		...
		if ((partition->usedMem + *size) > partition->availableMem) {
			proc_lockClear(&phmap_common.lock);
			return PHADDR_INVALID;
		}
		else {
			partition->usedMem += *size;
		}
		proc_lockClear(&phmap_common.lock);
	}
...
int vm_mappages(pmap_t *pmap, void *vaddr, addr_t pa, size_t size, vm_attr_t attr)
{
	...
	for (; i < size; i += SIZE_PAGE) {
		...
		while (pmap_enter(pmap, pa + i, vaddr + i, attr, ap) < 0) {
			tmp.addr = vm_phAlloc(&s, PAGE_OWNER_KERNEL | PAGE_KERNEL_PTABLE, MAP_CONTIGUOUS, NULL);
...
int _page_sbrk(pmap_t *kpmap, void **start, void **end)
{
	...
	addr = vm_phAlloc(&s, PAGE_OWNER_KERNEL | PAGE_KERNEL_HEAP, MAP_CONTIGUOUS, NULL);
...
static int _phmap_mapsInit(pmap_t *kpmap, void **bss, void **top, ph_entry_t *initEntries, addr_t addr)
{
	...
	do {
		...
		for (;;) {
			...
			LIB_ASSERT_ALWAYS(vm_phFree(page.addr, SIZE_PAGE, NULL) == EOK, "vm: Problem with freeing page during phmap init (addr=%p)", (void *)page.addr);
...
void _vm_phmap_init(pmap_t *kpmap, void **bss, void **top)
{
	...
	while (_pmap_kernelSpaceExpand(kpmap, &vaddr, (*top) + max((freesz + allocsz) / 4U, (1UL << 23)), p) != 0) {
		page.addr = vm_phAlloc(&s, PAGE_OWNER_KERNEL | PAGE_KERNEL_PTABLE, MAP_CONTIGUOUS, NULL);
