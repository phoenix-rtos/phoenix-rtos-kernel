...
void vm_pageFree(page_t *p)
{
	...
	while (((ptr_t)lh >= (ptr_t)pages_info.pages) && ((ptr_t)rh < ((ptr_t)pages_info.pages + (pages_info.allocsz + pages_info.freesz) / SIZE_PAGE)) && ((lh->flags & PAGE_FREE) != 0U) && ((rh->flags & PAGE_FREE) != 0U) && (lh->idx == rh->idx) && ((lh->addr + (1UL << lh->idx)) == rh->addr) && (idx < SIZE_VM_SIZES)) {
...
static int _page_map(pmap_t *pmap, void *vaddr, addr_t pa, unsigned int attr)
{
	page_t *ap = NULL;

	while (pmap_enter(pmap, pa, vaddr, (int)attr, ap) < 0) {
