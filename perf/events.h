...
extern void perf_traceEventsWrite(u8 cpuChan, u8 event, const void *data, size_t sz, u32 *ts);
...
static inline void perf_traceEventsMapAdd(const vm_map_t *map, size_t size)
{
	struct {
		u32 map;
		u32 size;
	...
	PERF_EVENT_BODY(PERF_EVENT_MAP_ADD, ev, NULL, {
		ev.map = (ptr_t)map;
		ev.size = size;
	});
}
...
static inline void perf_traceEventsMapRemove(const vm_map_t *map, size_t size)
{
	struct {
		u32 map;
		u32 size;
