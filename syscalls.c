...
int syscalls_sys_perf_start(void *ustack)
{
	process_t *proc = proc_current()->process;
	perf_mode_t mode;
	unsigned flags;
	...
	GETFROMSTACK(ustack, int, mode, 0);
...
int syscalls_sys_perf_read(void *ustack)
{
	process_t *proc = proc_current()->process;
	void *buffer;
	size_t sz;
...
int syscalls_sys_perf_stop(void *ustack)
{
	perf_mode_t mode;
...
int syscalls_sys_perf_finish(void *ustack)
{
	perf_mode_t mode;
...
int syscalls_msgPulse(void *ustack)
{
	...
	GETFROMSTACK(ustack, u32, port, 0);
	GETFROMSTACK(ustack, u8, pulse, 1);
