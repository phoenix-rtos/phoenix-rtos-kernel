...
int coredump_enqueue(process_t *process)
{
	...
	if ((thread->sigpend & (1u << SIGNULL)) == 0) {
		return -ENOENT;
	}
...
static int coredump_memEntryList(coredump_memseg_t *list, size_t n)
{
	map_entry_t *e;
	size_t segCnt = 0;
	size_t i = 0;
...
{
	const coredump_req_t *req = (coredump_req_t *)msg->i.data;
	...
	proc_sendFromMap(responsePort, &memMsg, coredump_common.current->mapp);
}
...
{
	coredump_memEntryList(msg->o.data, msg->o.size / sizeof(coredump_memseg_t));
	msg->o.err = EOK;
	proc_respond(coredump_common.oid.port, msg, rid);
}
...
static int coredump_threadCnt(process_t *process)
{
