...
static int coredump_handleMemRead(msg_t *msg, msg_rid_t rid)
{
	...
	ret = proc_sendFromProcess(responsePort, &memMsg, coredump_common.current);
...
static int coredump_handleRelocRead(msg_t *msg, msg_rid_t rid)
{
	...
	for (i = 0; i < process->relocsz && i < msg->o.size / sizeof(coredump_reloc); i++) {
		relocs[i].vbase = process->reloc[i].vbase;
		relocs[i].pbase = process->reloc[i].pbase;
	}
	hal_memset((char*)msg->o.data + i * sizeof(coredump_reloc), 0, msg->o.size - i * sizeof(coredump_reloc));
...
void coredump_dump(void)
{
	...
	resp = (coredump_general *)msg.o.data;
