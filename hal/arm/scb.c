...
static struct scb_s * const scb = (void *)0xe000e000;
...
void _hal_scbSetPriorityGrouping(u32 group)
{
	...
	scb->aircr = t | 0x5fa0000 | ((group & 7) << 8);
}


void _hal_scbSetPriority(s8 excpn, u32 priority)
{
	volatile u8 *ptr = (u8 *)&scb->shpr1 + excpn - 4;
...
{
	scb->aircr = ((0x5fau << 16) | (scb->aircr & (0x700u)) | (1u << 2));
...
void _hal_scbEnableDCache(void)
{
	...
	if ((scb->ccr & (1 << 16)) == 0) {
		...
		do {
			ways = (ccsidr >> 3) & 0x3ff;
			do {
				scb->dcisw = ((sets & 0x1ff) << 5) | ((ways & 0x3) << 30);
			} while (ways-- != 0);
		} while (sets-- != 0);
		hal_cpuDataSyncBarrier();
