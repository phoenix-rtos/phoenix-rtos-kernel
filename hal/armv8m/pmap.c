...
#define MPU_BASE ((void *)0xe000ed90U)
...
void pmap_switch(pmap_t *pmap)
{
	...
	if (pmap != NULL && pmap->hal != NULL) {
		...
		*(pmap_common.mpu + mpu_ctrl) &= ~1U;
		...
		for (i = 0; i < max(allocCnt, pmap_common.lastMPUCount); i += 4U) {
		...
		*(pmap_common.mpu + mpu_ctrl) |= 1U;
		...
		pmap_common.lastMPUCount = allocCnt;
...
{
	const unsigned int cnt = (syspage->hs.mpuType >> 8U) & 0xffU;
	unsigned int i;
	...
	pmap->hal = NULL;
	pmap_common.lastMPUCount = min(cnt, MPU_MAX_REGIONS);
	...
	if (cnt == 0U) {
		pmap_common.mpuEnabled = 0;
		return;
	}

	pmap_common.mpuEnabled = 1;
