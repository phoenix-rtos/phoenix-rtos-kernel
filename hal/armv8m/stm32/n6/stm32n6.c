...
int hal_platformctl(void *ptr)
{
	...
	switch (data->type) {
		...
		case pctl_perclk:
			if (data->action == pctl_get) {
				data->cpuclk.hz = _stm32_rccGetPerClock();
				ret = EOK;
			}
...
int _stm32_gpioConfig(unsigned int d, u8 pin, u8 mode, u8 af, u8 otype, u8 ospeed, u8 pupd)
{
	...
	if ((d < pctl_dev_gpioa) || (d > pctl_dev_gpioq) || (pin > 15)) {
		return -1;
	}

	base = stm32_common.gpio[d - pctl_dev_gpioa];
...
int _stm32_gpioSetPort(unsigned int d, u16 val)
{
	...
	if ((d < pctl_dev_gpioa) || (d > pctl_dev_gpioq)) {
		return -1;
	}
...
void _stm32_init(void)
{
	...
	static const int gpioDevs[] = {
		pctl_dev_gpioa, pctl_dev_gpiob, pctl_dev_gpioc, pctl_dev_gpiod,
		pctl_dev_gpioe, pctl_dev_gpiof, pctl_dev_gpiog, pctl_dev_gpioh,
		pctl_dev_gpion, pctl_dev_gpioo, pctl_dev_gpiop, pctl_dev_gpioq
	};
