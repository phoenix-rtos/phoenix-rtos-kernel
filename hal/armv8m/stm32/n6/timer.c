...
static struct {
	...
	u32 ticksPerMicrosecond;
	u32 ticksPerInterval;
} timer_common;
...
void timer_jiffiesAdd(time_t t)
{
	...
	timer_common.ticks += t * timer_common.ticksPerMicrosecond;
...
{
	return hal_getTicks() / timer_common.ticksPerMicrosecond;
}
...
void _hal_timerInit(u32 interval)
{
	...
	timer_common.ticksPerMicrosecond = _stm32_rccGetCPUClock() / 1000;
	timer_common.ticksPerInterval = ((u64)_stm32_rccGetCPUClock() * interval) / 1000000;
	if (timer_common.ticksPerInterval > 0x01000000) {
		return;
	}
