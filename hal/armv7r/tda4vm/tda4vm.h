...
extern int tda4vm_getPLL(unsigned int pll, tda4vm_clk_pll_t *config);
...
extern u64 tda4vm_getFrequency(unsigned int pll, unsigned int hsdiv);

/* Do warm reset. Software POR is not possible on this platform. */
extern void tda4vm_warmReset(void);


extern int tda4vm_setPinConfig(unsigned int pin, const tda4vm_pinConfig_t *config);


extern int tda4vm_getPinConfig(unsigned int pin, tda4vm_pinConfig_t *config);


extern int tda4vm_setDebounceConfig(unsigned int idx, unsigned int period);
...
extern int tda4vm_RATMapMemory(unsigned int entry, addr_t cpuAddr, u64 physAddr, u32 logSize);


extern void tda4vm_RATUnmapMemory(unsigned int entry);


extern int tda4vm_setClksel(unsigned int sel, unsigned int val);


/* Returns < 0 on error, otherwise the clksel setting */
extern int tda4vm_getClksel(unsigned int sel);


extern int tda4vm_setClkdiv(unsigned int sel, unsigned int val);


/* Returns < 0 on error, otherwise the clksel setting */
extern int tda4vm_getClkdiv(unsigned int sel);
