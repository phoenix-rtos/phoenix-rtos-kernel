...
static int _zynqmp_parseReset(int dev, volatile u32 **reg, u32 *bit)
{
	...
	if ((dev < pctl_devclock_lpd_gem0) || (dev > pctl_devreset_fpd_ddr_reserved)) {
		return -1;
	}
...
void _hal_platformInit(void)
{
	...
	zynq_common.csu = _pmap_halMapDevice(CSU_BASE_ADDRESS, 0, SIZE_PAGE);
	...
	zynq_common.ddrc = _pmap_halMapDevice(DDRC_BASE_ADDRESS, 0, SIZE_PAGE);
	zynq_common.ddr_phy = _pmap_halMapDevice(DDR_PHY_BASE_ADDRESS, 0, SIZE_PAGE);
