 ...
 *
 * Exception and interrupt handling for ZynqMP
 *
...
#define DEFAULT_CPU_MASK ((1 << NUM_CPUS) - 1)
...
void interrupts_setCPU(unsigned int irqn, u32 cpuID)
{
	...
	*(interrupts_common.gicd + gicd_itargetsr0 + irq_reg) = mask | ((cpuID & 0xff) << irq_offs);
}
...
static int _zynqmp_interrupts_classify(unsigned int irqn)
{
	...
	if ((irqn < 40) || ((irqn >= 129) && (irqn <= 135))) {
		...
		return cfg_rising_edge;
	}
	else {
		return cfg_high_level;
	}
}
...
void _hal_interruptsInit(void)
{
	...
	dtb_getGIC(&version, &gicc, &gicd);
	if ((version < 1) || (version > 2)) {
		return;
	}

	interrupts_common.gicd = _pmap_mapDevice(gicd, SIZE_PAGE);
	interrupts_common.gicc = _pmap_mapDevice(gicc, SIZE_PAGE);
...
void _hal_interruptsInitPerCPU(void)
{
	...
	*(interrupts_common.gicc + gicc_ctlr) = *(interrupts_common.gicc + gicc_ctlr) | 0xf;
}
