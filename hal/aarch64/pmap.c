...
static void _pmap_asidAlloc(pmap_t *pmap)
{
	...
	if (pmap_common.firstFreeAsid == N_ASIDS) {
		...
		for (i = assigned / 64U; i < N_ASID_MAP; i++) {
			...
			if (free != 0U) {
				pmap_common.firstFreeAsid = (unsigned int)i * 64U + hal_cpuGetFirstBit(free);
				break;
			}
		}

		if (i == N_ASID_MAP) {
			pmap_common.firstFreeAsid = (unsigned int)N_ASIDS;
		}
...
static int _pmap_enter(pmap_t *pmap, addr_t pa, void *vaddr, int attr, page_t *alloc)
{
	unsigned lvl;
	descr_t *tt;
	descr_t entry;
...
int pmap_enter(pmap_t *pmap, addr_t pa, void *vaddr, int attr, page_t *alloc)
{
	...
	hal_spinlockSet(&pmap_common.lock, &sc);
	ret = _pmap_enter(pmap, pa, vaddr, attr, alloc);
	hal_spinlockClear(&pmap_common.lock, &sc);
	return ret;
}
